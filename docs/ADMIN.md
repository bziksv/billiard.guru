...
Формат **`FIXED_SWISS_64`** — **эталон** LLB для 64 участников: **111 встреч**, 10 колонок, нижняя тур 1–4, oлимпийка с 1/8. Регламент: [`BRACKET_REFERENCE_64_32.md`](./BRACKET_REFERENCE_64_32.md).

Формат **`FIXED_SWISS_64_BRONZE`** — тот же эталон + **#112** (матч за 3–4 под финалом). Подпись: «Сетка на 64 до 2 поражений, oлимпийка с 1/8 с определением 3 и 4 места (доп.игра)».
