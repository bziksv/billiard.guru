...
const FIXED_SWISS_8_FORMATS = new Set([
  ...
  "FIXED_PAIR_SWISS_8R2_1_3_mesto",
]);
...
export function getDefaultBracketParticipantRules(
  ...
  if (code === "FIXED_SWISS_128R8_2_3_mesta") {
    return {
      min: 128,
      max: 128,
      exact: 128,
      label: "ровно 128",
      hint:
        "Сетка «до 2 поражений» на 128 участников — 247 встреч, олимпийка с 1/8: проигравшие в 1/8 сразу на места 33–48. " +
        "Добавьте участников до 128 или смените формат.",
    };
  }
  if (code === "FIXED_SWISS_128R8_1_3_mesto") {
    return {
      min: 128,
      max: 128,
      exact: 128,
      label: "ровно 128",
      hint:
        "Сетка «до 2 поражений» на 128 участников — 248 встреч (247 + матч за 3–4 #248), олимпийка с 1/8: проигравшие в 1/8 сразу на места 33–48. " +
        "Добавьте участников до 128 или смените формат.",
    };
  }
  if (code === "FIXED_SWISS_256R8_1_3_mesto") {
    return {
      min: 2,
      max: 256,
      label: "2–256",
      hint:
        ...
        "Добавьте участников или смените формат.",
    };
  }
  if (code === "FIXED_SWISS_64R8_2_3_mesta") {
    return {
      min: 64,
      max: 64,
      exact: 64,
      label: "ровно 64",
      hint:
        "Сетка «до 2 поражений» на 64 участника — 111 встреч, олимпийка с 1/8: проигравшие в 1/8 (#81–#88) сразу на места 17–24, без нижней тур 4. " +
        "Добавьте участников до 64 или смените формат.",
    };
  }
  if (code === "FIXED_SWISS_64R8_1_3_mesto") {
    return {
      ...
      label: "ровно 64",
      hint:
        "Сетка «до 2 поражений» на 64 участника — 112 встреч (111 + матч за 3–4 #120), олимпийка с 1/8: проигравшие в 1/8 (#81–#88) сразу на места 17–24. " +
        "Добавьте участников до 64 или смените формат.",
    };
  }
  ...
  if (code === "FIXED_SWISS_32R8_2_3_mesta") {
    return {
      ...
      label: "ровно 32",
      hint:
        "Сетка «до 2 поражений» на 32 участника — 55 встреч, олимпийка с 1/8: проигравшие в 1/8 (#41–#44) сразу на места 9–12, без нижней тур 4. " +
  ...
  if (code === "FIXED_SWISS_32R8_1_3_mesto") {
    return {
      ...
      label: "ровно 32",
      hint:
        "Сетка «до 2 поражений» на 32 участника — 56 встреч (55 + матч за 3–4 #60), олимпийка с 1/8: проигравшие в 1/8 (#41–#44) сразу на места 9–12. " +
        "Добавьте участников до 32 или смените формат.",
    };
  }
  ...
  if (FIXED_SWISS_8_FORMATS.has(code)) {
    const is8R2Bronze =
      ...
      code === "FIXED_PAIR_SWISS_8R2_1_3_mesto";
    return {
      min: 8,
      max: 8,
      exact: 8,
      label: "ровно 8",
      hint: is8R2Bronze
        ? "Сетка «до 2 поражений» на 8 участников — 14 встреч, олимпийка с 1/2 (#13 за 3–4, #14 финал). " +
  ...
  if (FIXED_SWISS_16_FORMATS.has(code)) {
    ...
    return {
      ...
      label: "ровно 16",
      hint: is16R2Bronze
        ? "Сетка «до 2 поражений» на 16 участников — 24 встречи (23 + матч за 3–4 #28), олимпийка с 1/2. " +
        ...
        : is16R4Bronze
          ...
          : code === "FIXED_SWISS_16R4_2_3_mesta" ||
            ...
            ? "Сетка «до 2 поражений» на 16 участников — 27 встреч, олимпийка с 1/4, проигравшие полуфиналисты делят 3-е место. " +
              "Добавьте участников до 16 или смените формат."
            : baseHint,
    };
  }
